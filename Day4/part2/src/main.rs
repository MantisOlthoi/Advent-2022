...
fn main() {
    ...
    for line_result in lines {
        ...
        if (second_elf_start >= first_elf_start && second_elf_start <= first_elf_end)
            || (second_elf_end >= first_elf_start && second_elf_end <= first_elf_end)
            || (first_elf_start >= second_elf_start && first_elf_start <= second_elf_start)
            || (first_elf_end >= second_elf_start && first_elf_end <= second_elf_end)
        {
            count += 1;
        }
    }
